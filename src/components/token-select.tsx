"use client";
import { tokensAtom } from "@/atoms/tokens";
...
export const TokenSelect = React.memo(
  ({ value, onChange }: TokenSelectProps) => {
    ...
    const queryBalances = React.useMemo(
      () =>
        tokens.map((token) => ({
    ...
    const { data: balances } = useContractReads({
      contracts: queryBalances,
      enabled: !!address,
    });

    const [search, setSearch] = React.useState("");

    const tokenWithBalance = React.useMemo(() => {
      return [
        {
          name: "Ether",
          symbol: "ETH",
    ...
    }, [balances, data?.formatted, tokens]);
    ...
    const filteredTokens = React.useMemo(() => {
      ...
      return tokenWithBalance.filter((token) => {
    ...
    }, [search, tokenWithBalance]);

    const currentToken = React.useMemo(
      () => tokenWithBalance.find((token) => token.address === value),
      [tokenWithBalance, value]
    );
          ...
          <div className="pt-[22px] px-5 pb-5 border-b-px border-[#363A45]">
            ...
            <label className="flex items-center gap-2 py-3 px-[14px] mt-6 border-px border-[#363A45] rounded-md cursor-text focus-within:border-[#414651] transition-colors">
              ...
              <input
                ...
                className="flex-auto text-sm leading-3 placeholder:text-secondary"
                value={search}
